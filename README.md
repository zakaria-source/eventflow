...
**Production-style event-driven order processing platform** built to demonstrate backend engineering decisions beyond CRUD: transactional consistency, asynchronous messaging, concurrent idempotency, retries, observability, integration testing and containerized deployment.
...
The row may be sent again. The consumer's event-ID claim makes re-delivery safe.
...
- [ ] Multi-instance outbox locking with `SKIP LOCKED`
...
3. At-least-once Kafka semantics and why transport-level "exactly once" is not the business guarantee.
4. Why `exists -> insert` is racy and how the atomic event claim fixes it.
5. Retry/DLT behavior under failures.
6. Domain isolation from infrastructure.
7. What each Testcontainers test proves — and what it does not prove.
8. Observability boundaries and why trace propagation through an outbox needs explicit design.
9. Kubernetes readiness/liveness and Prometheus metrics.
10. Changes needed at 10× and 100× traffic.
